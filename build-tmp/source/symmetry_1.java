...
public class symmetry_1 extends PApplet {
...
public void horizontalLines(float y0, int qty, float spread) {
	float deltaY = 0.0f;
	for(int c=0;c<qty;c++) {
		int sat = 10;
		int brt = 90;
		if (c == 0) {
			...
			int myColor = color((360/qty)*c, (sat*c)/2, brt);
		...
		int rand1 = PApplet.parseInt(random(1, spread));
		deltaY += rand1;
...
public void verticalLines(int sets) {
	for(int v=1;v<=sets;v++) {
		int y0 = -200;
		int x0 = PApplet.parseInt(random(100,400));
		for(int c=0;c<10;c++) {
			stroke(255-(c*(255/10)));
			strokeWeight(1);
			int rand1 = PApplet.parseInt(random(10, 75));
			int rand2 = PApplet.parseInt(random(10, 20));
			int x1 = x0+(c*rand1);
			int x2 = x0-(c*rand2);
			line(x1, y0, x2, width);
			int symX1 = width-x1;
...
public void dots(float growthFactor, int steps, int lines, int y0) {

	noStroke();
	int x0 = width/2;
	for(int b=0;b<lines;b++) {
		int deltaY = (height-y0-PApplet.parseInt(growthFactor*steps))/lines;
...
public void draw() {
	...
	// rotate(radians(10));
	float y0 = PApplet.parseInt(random(100,200));
	for(float c=1;c<=5;c++) {
		y0 = y0+random(y0);
		println("y0: "+y0);
...
public void mousePressed() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "symmetry_1" };
    if (passedArgs != null) {
